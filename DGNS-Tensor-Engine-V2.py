#Updated code version, trying to fix bugs when running Torch on Colab...
import numpy as np
import torch
...
def compute_sgs_term(S, dx):
    ...
    nu_t = (Cs * dx)**2 * S_mag
    # Pad nu_t to match laplacian shape
...
def navier_stokes_step(S, dt, nu, rho, M, N, dx, use_dst=False):
    ...
    w_new = v[1:-1, 1:-1, 1:-1] + dt * (-conv_w + effective_nu * lap_w)
...
for iteration in range(max_iterations):
    ...
    N_fine = 64  # Memory-friendly for Colab
