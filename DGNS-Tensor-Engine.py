...
Cs = 0.1
max_iterations = 1  # Reduced to fit Colab
...
def spectral_smooth(S, N, dx):
    for i in range(3):
        ...
        kx, ky, kz = torch.meshgrid(k, k, k, indexing='ij')
        k2 = kx**2 + ky**2 + kz**2
        filter_mask = (k2 < (N * np.pi / 4)**2).float()  # Adjusted cutoff
...
def refine_subgrid(S, M, N_coarse, dx_coarse, N_fine, dx_fine, region, nu, rho):
    ...
    for t in range(100):  # Reduced steps
...
for iteration in range(max_iterations):
    ...
    N_fine = 128  # Memory-friendly for Colab
...
# 3D Voxel Plot of Negative Uncomputable Space
